// LatticeLoader from React Bits (JS + CSS variant), ported to TypeScript.
// The logic and CSS are unchanged; only types were added so it fits this TS project.
...
export type LatticeStatus = "working" | Mark
...
export type LatticePattern =
  | string
  | { cells: (number | null)[]; loop?: number; scale?: number; lit?: 0.25 | 0.35 | 0.45 | 0.62 }

const PATTERNS: Record<string, Partial<Record<Grid, ResolvedPattern>>> = {
...
const MARKS: Record<Grid, Record<Mark, number[]>> = {
  3: { done: [2, 3, 5, 7], error: [0, 2, 4, 6, 8] },
  4: { done: [7, 8, 10, 13], error: [0, 3, 5, 6, 9, 10, 12, 15] },
}

const resolvePattern = (pattern: LatticePattern, grid: Grid): ResolvedPattern => {
...
export interface LatticeLoaderProps {
  /** The verb while working. */
  label?: string
  /** The verb after status turns to done; the frozen time follows it. */
  doneLabel?: string
  /** The verb after status turns to error. */
  errorLabel?: string
  /** Drives everything: the wave runs, or freezes into a check or a cross while the stopwatch stops. */
  ...
  glowColor?: string
  showTimer?: boolean
  /** Controlled elapsed seconds. When set, the internal clock never runs. */
  elapsed?: number
  className?: string
  style?: CSSProperties
}
...
export default function LatticeLoader({
  ...
  const markRef = useRef<Mark>("done")
  const mark: Mark = status === "working" ? markRef.current : status
